...
def configure_ngrok():
    ngrok_token = "2sebhtbK3KHplVb2A1N88AoTHA6_PLVnf4LhUMx9HkYDehdC"
    try:
...
def update_soul_txt(public_url):
    ...
    print(f"New ngrok link saved in shivam.txt")

def update_vps_soul_txt(public_url):
    vps_ip = "66.135.6.59"
    vps_user = "master_hrgvkmcnwe"
    vps_password = "CQ8PZwBJDmuB"
    ...
    try:
        ...
        sftp = ssh.open_sftp()
        with sftp.open("shivam.txt", "w") as file:
            file.write(public_url)
        sftp.close()
        ssh.close()
        print("Updated shivam.txt on VPS successfully.")
    except Exception as e:
        print(f"Failed to update shivam.txt on VPS: {str(e)}")
